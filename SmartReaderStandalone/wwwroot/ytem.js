...
const columnDefs = [
    { field: 'barcode', headerName: 'Ordem', cellStyle: { fontSize: '20px' }, width: 300 },
    { field: 'sku', headerName: 'Produto', cellStyle: { fontSize: '20px' }, width: 300 },
...
function confirmData() {
    ...
    if (this.orderBarcode.length === 0 || this.contentFormat.length === 0
        ...
        proceed = false;
        swal("Enviar Ordem ", "Dados da ordem não encontrados.", "error");
    }
    ...
    if (this.containsUnexpected) {
        swal("Foram identificados itens inesperados, deseja enviar mesmo assim?", {
            buttons: {
                cancel: "Não",
                send: {
                    text: "Enviar!",
...
function publishData() {
    ...
    fetch(url, {
        ...
        .then((data) => {
            console.log('data.length: ' + data.length);
            swal("Ordem " + this.orderBarcode, "Dados enviados com sucesso. ", "success");

        })
        .catch(function (error) {
            console.log('There has been a problem with your fetch operation: ' + error.message);
            swal("Ordem " + this.orderBarcode, "Erro no envio dos dados. " + error.message, "error");
        });
...
function finishOrder() {
    ...
    fetch(url, {
        ...
        .then((data) => {
            console.log('data.length: ' + data.length);
            swal("Ordem " + this.orderBarcode, "Ordem encerrada com sucesso. ", "success");
